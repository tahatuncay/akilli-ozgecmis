  ...
  return (
    <div className="w-full min-h-full bg-white text-gray-900 p-10 font-sans flex flex-col text-[11px] leading-relaxed">
      ...
      <header className="flex flex-col items-center text-center border-b-[3px] border-gray-900 pb-6 mb-6">
        <h1 className="text-4xl font-black uppercase tracking-[0.15em] mb-2 text-gray-900">{personalInfo.fullName || labels.fullNamePlaceholder}</h1>
        <h2 className="text-sm font-semibold tracking-[0.2em] text-gray-500 uppercase mb-4">{personalInfo.title || labels.titlePlaceholder}</h2>
      ...
      <div className="flex flex-col gap-6">
        ...
        {summary && (
          <section>
            <h3 className="text-xs font-bold uppercase tracking-widest text-gray-900 border-b border-gray-300 pb-2 mb-3">{labels.summary}</h3>
        ...
        {experiences.length > 0 && (
          <section>
            <h3 className="text-xs font-bold uppercase tracking-widest text-gray-900 border-b border-gray-300 pb-2 mb-4">{labels.professionalExperience}</h3>
        ...
        <div className="grid grid-cols-2 gap-8 mt-2">
          ...
          {education.length > 0 && (
            <section>
              <h3 className="text-xs font-bold uppercase tracking-widest text-gray-900 border-b border-gray-300 pb-2 mb-4">{labels.educationHistory}</h3>
          ...
          <div className="flex flex-col gap-6">
            ...
            {skills.length > 0 && (
              <section>
                <h3 className="text-xs font-bold uppercase tracking-widest text-gray-900 border-b border-gray-300 pb-2 mb-4">{labels.coreSkills}</h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              {languages.length > 0 && (
                <section>
                  <h3 className="text-xs font-bold uppercase tracking-widest text-gray-900 border-b border-gray-300 pb-2 mb-3">{labels.languages}</h3>
                  <ul className="flex flex-col gap-1.5">
                    {languages.map((lang) => (
                      <li key={lang.id} className="flex justify-between items-center">
                        <span className="text-gray-800">{lang.name}</span>
                        <span className="text-gray-500 text-[10px]">{labels.langLevels[lang.proficiency] || lang.proficiency}</span>
                      </li>
                    ))}
                  </ul>
                </section>
              ...
              {certificates.length > 0 && (
                <section>
                  <h3 className="text-xs font-bold uppercase tracking-widest text-gray-900 border-b border-gray-300 pb-2 mb-3">{labels.certificates}</h3>
